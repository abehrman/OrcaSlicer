...
std::string WipeTowerIntegration::append_tcr(GCode& gcodegen, const WipeTower::ToolChangeResult& tcr, int new_extruder_id, double z) const
{
    if (new_extruder_id != -1 && new_extruder_id != tcr.new_tool) {
        // Diagnostic dump: list this tower's generated toolchanges (nominal + local-Z) for the
        // current layer (initial->new), its anchor position (tower 0 sits at wipe_tower_x/y; the
        // rest are offset in +X), and the loaded tool — so one failing slice reveals whether the
        // requested change is missing, misordered, or routed to the wrong tower.
        auto dump = [this](const std::vector<std::vector<WipeTower::ToolChangeResult>> &changes) {
            std::string s;
            if (m_layer_idx >= 0 && size_t(m_layer_idx) < changes.size())
                for (const WipeTower::ToolChangeResult &t : changes[size_t(m_layer_idx)])
                    s += Slic3r::format(" %1%->%2%", t.initial_tool, t.new_tool);
            return s;
        };
        const int cur_tool = gcodegen.writer().extruder() ? int(gcodegen.writer().extruder()->id()) : -1;
        throw Slic3r::InvalidArgument(Slic3r::format(
            "Error: WipeTowerIntegration::append_tcr unexpected toolchange: layer_idx=%1% tool_change_idx=%2% requested=%3% expected=%4% initial=%5% current_tool=%6% tower_pos=(%7%,%8%) n_layers=%9% nominal={%10% } local_z={%11% }",
            m_layer_idx, m_tool_change_idx, new_extruder_id, tcr.new_tool, tcr.initial_tool, cur_tool,
            m_wipe_tower_pos.x(), m_wipe_tower_pos.y(), m_tool_changes.size(), dump(m_tool_changes), dump(m_local_z_tool_changes)));
    }
